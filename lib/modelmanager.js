...
// Гарантированно-бесплатная ли модель для автодобавления.
// :free (OpenRouter) — всегда. Нatивные источники — только известные бесплатные
// семейства; иначе риск добавить платную модель, на которой авто-тест прошёл
// (mistral-medium, deepseek-v4-pro, GLM-5.2), и она попадёт в роутинг.
// Источники с большим лимитом (groq/cerebras/gemini) реально бесплатны — там
// семейства допустимы. Источники с малым лимитом и платными рядами (hf/nim)
// — только явно безопасные free-семейства (gemma/qwen/llama/nano/ministral).
function isAutoAddable(candidate) {
  const model = String(candidate.model || '');
  if (model.includes(':free')) return true;
  const source = candidate.source;
  // Явно платные модели — никогда (даже если авто-тест прошёл на балансе).
  if (/(mistral-medium|mistral-large|magistral|deepseek-v4-pro|GLM-5\.|-v4-pro\b)/i.test(model)) return false;
  if (['groq', 'cerebras', 'gemini'].includes(source)) {
    return /(qwen|llama|gemma|gpt-oss|nemotron|mini|compound)/i.test(model);
  }
  if (source === 'nim') {
    // NIM — стабильно бесплатный тариф: какие-то из этих семейств по free.
    return /(nemotron|llama|kimi|minimax|laguna|gemma|qwen|deepseek)/i.test(model);
  }
  // hf и прочие осторожные — только проверенные free-семейства
  return /(gemma|qwen|llama-3\.1|ministral|nano)/i.test(model);
}
...
class ModelManager {
  ...
  async scanAndAdd() {
    ...
    for (const c of tested) {
      ...
      // Анти-мусор: автодобавляем только гарантированно-бесплатные модели.
      // OpenRouter — только :free. Остальные источники (groq/cerebras/gemini/
      // nim/hf) — только если мы уже знаем модель как бесплатную (есть в
      // исходном каталоге/белом списке) или это явно free. Иначе не добавляем:
      // авто-тест может пройти на платной модели и она попадёт в роутинг.
      if (!isAutoAddable(c)) {
        this.log(`  ⏭ ${c.model}: не бесплатная — пропуск (report-only)`);
        continue;
      }
...
module.exports = { ModelManager, keyFor, inferSource, isAutoAddable, DEFAULTS };
