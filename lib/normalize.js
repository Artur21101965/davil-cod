...
const MAX_LEN = 2000;
const TAIL_LEN = 400; // последние N символов диалога — всегда в ключе, чтобы
                 // два диалога с общим длинным префиксом не схлопывались
                 // в один кэш-ключ (иначе кэш отдаёт ответ из чужого чата).
...
function normalizeText(text) {
  const cleaned = (text || '')
    ...
    .trim();
  // Хэш-ключ = начало (первые MAX_LEN) + конец (последние TAIL_LEN).
  // Если просто обрезать до MAX_LEN, два длинных диалога с одинаковым
  // префиксом совпадут — кэш отдаст ответ из чужого чата. Хвост диалога
  // (последние сообщения) почти всегда различает их.
  const tail = cleaned.length > MAX_LEN
    ? cleaned.slice(-TAIL_LEN)
    : '';
  return cleaned.slice(0, MAX_LEN) + tail;
}

module.exports = { normalizeMessages, normalizeText, looksLikeCode, MAX_LEN, TAIL_LEN };
