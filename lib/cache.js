...
const MAX_ENTRY_BYTES = 64 * 1024; // don't persist responses larger than 64KB
    ...
    try {
      ...
      if (data && Array.isArray(data.entries)) {
        ...
        for (const e of data.entries) {
          ...
          this.cache.set(e.key, { value: e.value, created: e.created });
        }
      }
    } catch {}
  }
    ...
    try {
      ...
      for (const [key, entry] of this.cache) {
        ...
        try {
          ...
          entries.push({ key, value: entry.value, created: entry.created });
        } catch {}
      }
      // Keep only the most recent 300 on disk to bound file size
      const trimmed = entries.slice(-300);
      fs.writeFileSync(CACHE_PATH, JSON.stringify({ saved: Date.now(), entries: trimmed }));
    } catch {}
  }
