...
const CHARS_PER_TOKEN = 1.5;      // heuristic chars→tokens. BIGGER делитель = МЕНЬШЕ оценка
                                  // (chars/1.5) = компакция РЕЖЕ (порог достигается позже).
                                  // Это консервативно к окнам моделей но лечит «замирание»:
                                  // каждое сжатие — отдельный LLM-вызов перед ответом, а при
                                  // частом срабатывании агент «перестаёт делать посередине».
                                  // Компакции в стоке начинали идти на 86% запросов → вернул 1.5.
