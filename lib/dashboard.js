...
<body>
  ...
  <div class="grid">
    ...
    <div class="card"><h2>База моделей</h2><div id="modeldb">Загрузка...</div></div>
  ...
  <script>
    ...
    async function refresh() {
      try {
        const [statsRes, recentRes, rpmRes, modelDbRes] = await Promise.all([
          fetch(_api('/v1/stats')), fetch(_api('/v1/recent')), fetch(_api('/v1/rpm')),
          fetch(_api('/v1/models-db')).catch(() => null)
        ]);
        ...
        renderRpm(rpmData);
        renderRecent(recent);
        if (modelDbRes && modelDbRes.ok) renderModelsDb(await modelDbRes.json());
    ...
    function renderContext(sum) {
      ...
      if (sum.tasks && sum.taskTotal > 0) {
        const pct = (n) => Math.round((n / sum.taskTotal) * 100) + '%';
        const parts = ['coding', 'reasoning', 'search', 'chat']
          .filter(k => sum.tasks[k] > 0)
          .map(k => '<div class="stat"><span class="stat-label">Категория: ' + k + '</span><span class="stat-value">' + sum.tasks[k] + ' (' + pct(sum.tasks[k]) + ')</span></div>');
        html += parts.join('');
      }
    ...
    function renderModelsDb(dbData) {
      if (!dbData || !dbData.stats) { document.getElementById('modeldb').innerHTML = 'Нет данных'; return; }
      const s = dbData.stats;
      const fmt = (obj) => Object.entries(obj || {}).map(([k, v]) => k + ': ' + v).join(', ') || '—';
      let html = '<div class="stat"><span class="stat-label">Всего моделей</span><span class="stat-value">' + (s.total || 0) + '</span></div>';
      html += '<div class="stat"><span class="stat-label">Статусы</span><span class="stat-value" style="font-size:11px">' + fmt(s.byStatus) + '</span></div>';
      html += '<div class="stat"><span class="stat-label">Источники</span><span class="stat-value" style="font-size:11px">' + fmt(s.bySource) + '</span></div>';
      if (dbData.manager) {
        html += '<div class="stat"><span class="stat-label">Автопоиск</span><span class="stat-value">' + (dbData.manager.enabled ? 'каждые ' + dbData.manager.intervalHours + 'ч' : 'выключен') + '</span></div>';
      }
      if (Array.isArray(dbData.top) && dbData.top.length > 0) {
        html += '<div style="margin-top:8px;font-size:11px;color:#888;">Топ по скору:</div>';
        for (const m of dbData.top.slice(0, 5)) {
          const color = m.status === 'active' ? '#00ff88' : m.status === 'dead' ? '#ff4444' : '#888';
          html += '<div class="stat"><span class="stat-label">' + m.key + ' (' + m.category + ')</span><span class="stat-value" style="color:' + color + '">' + m.score + '</span></div>';
        }
      }
      document.getElementById('modeldb').innerHTML = html;
    }
    ...
    function fmtTokens(n) {
      ...
      const k = n / 1000;
      const s = k < 10 ? (Math.round(k * 10) / 10).toFixed(1).replace(/\.0$/, '') : Math.round(k).toString();
      return s + 'K';
    }
    ...
    function renderTokens(tokens) {
      ...
      for (const [key, t] of Object.entries(tokens)) {
        html += '<div class="stat"><span class="stat-label">' + key + '</span><span class="stat-value">' + fmtTokens(t.totalTokens) + ' ток.</span></div>';
      }
    ...
    function renderSetup(data) {
      ...
      for (const [envVar, info] of Object.entries(data.groups)) {
        const maskedVal = data.masked && data.masked[envVar] || '';
    ...
    async function validateKey(envVar) {
      ...
      // Пустое поле → сервер проверит реальный сохранённый ключ из .env.
      status.className = 'status-wait'; status.textContent = 'Проверка...';
      try {
        const url = _api('/v1/setup/validate?envVar=' + encodeURIComponent(envVar)) + (key ? '&apiKey=' + encodeURIComponent(key) : '');
