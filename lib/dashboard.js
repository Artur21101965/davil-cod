  ...
  <script>
    async function refresh() {
      try {
        const [statsRes, recentRes, rpmRes] = await Promise.all([
          fetch('/v1/stats'), fetch('/v1/recent'), fetch('/v1/rpm')
        ]);
    ...
    function renderProviders(health) {
      ...
      for (const [key, h] of Object.entries(health)) {
        ...
        const statusText = h.status === 'up' ? 'Работает' : h.status === 'error' ? 'Ошибка' : 'Неизвестно';
        html += '<div class="provider"><div class="status-dot ' + cls + '"></div>';
        html += '<span>' + key + ' (' + statusText + ')</span><span class="latency">' + h.latency_ms + 'ms (счёт: ' + h.score + ')</span></div>';
      }
