...
<head>
  ...
  <link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'%3E%3Ccircle cx='8' cy='8' r='7' fill='%2300ff9d'/%3E%3C/svg%3E">
  <title>Freegate — Панель управления</title>
  <style>
    /* ── Aesthetic: авиа-приборная панель / военный CRT-дисплей ── */
    :root {
      --bg: #06080d; --panel: #0b0f16; --panel2: #10151f; --border: #1b2533;
      --accent: #00ff9d; --accent-dim: #00b878; --accent-ink: #00ff9d;
      --ok: #00ff9d; --warn: #ffb454; --err: #ff5c5c;
      --text: #d7e2e8; --muted: #5d6d7d;
      --scan: rgba(0,255,157,.03); --grid: rgba(0,255,157,.05);
    }
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'SF Mono', 'JetBrains Mono', ui-monospace, Menlo, monospace; background: var(--bg); color: var(--text); padding-bottom: 40px;
      background-image:
        radial-gradient(ellipse at 20% -10%, rgba(0,255,157,.07), transparent 55%),
        radial-gradient(ellipse at 90% 0%, rgba(0,120,255,.05), transparent 50%),
    ...
    body::after { content: ''; position: fixed; inset: 0; pointer-events: none; z-index: 9999;
      background: repeating-linear-gradient(0deg, var(--scan) 0px, var(--scan) 1px, transparent 2px, transparent 4px); }

    /* ── Sticky KPI-полоска ── */
    #topbar { position: sticky; top: 0; z-index: 10; background: rgba(6,8,13,.82); backdrop-filter: blur(8px) saturate(120%);
      border-bottom: 1px solid var(--border); padding: 12px 20px; display: flex; flex-wrap: wrap; gap: 10px; align-items: center;
      box-shadow: 0 4px 24px rgba(0,0,0,.55); }
    .brand { font-size: 16px; font-weight: 700; color: var(--accent); display: flex; align-items: center; gap: 9px; margin-right: 10px;
      letter-spacing: .5px; text-shadow: 0 0 18px rgba(0,255,157,.45); }
    .brand::after { content: ''; width: 1px; height: 18px; background: var(--border); margin-left: 4px; }
    .dot { width: 9px; height: 9px; border-radius: 50%; display: inline-block; background: var(--muted); flex-shrink: 0; }
    .dot.ok { background: var(--ok); box-shadow: 0 0 10px var(--ok), 0 0 2px var(--ok); animation: pulse 2s ease-in-out infinite; }
    .dot.bad { background: var(--err); box-shadow: 0 0 10px var(--err); animation: pulse 1s ease-in-out infinite; }
    @keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: .5; } }
    .kpi { display: flex; flex-wrap: wrap; gap: 8px; }
    .kpi-chip { background: linear-gradient(180deg, var(--panel), var(--panel2)); border: 1px solid var(--border); border-radius: 6px;
      padding: 6px 12px; font-size: 11px; color: var(--muted); letter-spacing: .4px; text-transform: uppercase;
      border-left: 2px solid var(--accent-dim); animation: rise .5s ease both; }
    .kpi-chip:nth-child(2) { animation-delay: .04s; } .kpi-chip:nth-child(3) { animation-delay: .08s; }
    .kpi-chip:nth-child(4) { animation-delay: .12s; } .kpi-chip:nth-child(5) { animation-delay: .16s; }
    .kpi-chip:nth-child(6) { animation-delay: .2s; } .kpi-chip:nth-child(7) { animation-delay: .24s; }
    @keyframes rise { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: none; } }
    .kpi-chip b { color: var(--accent); font-size: 15px; margin-left: 7px; font-weight: 700; }
    .kpi-chip.warn b { color: var(--warn); } .kpi-chip.warn { border-left-color: var(--warn); }
    .kpi-chip.err b { color: var(--err); } .kpi-chip.err { border-left-color: var(--err); }

    /* ── Табы ── */
    #tabs { display: flex; gap: 4px; padding: 14px 20px 0; flex-wrap: wrap; }
    .tab { padding: 10px 20px; border-radius: 6px 6px 0 0; cursor: pointer; background: var(--panel);
      color: var(--muted); border: 1px solid var(--border); border-bottom: none; font-size: 12.5px; user-select: none;
      letter-spacing: .3px; transition: color .15s ease, background .15s ease; position: relative; }
    .tab.active { color: var(--accent); background: linear-gradient(180deg, rgba(0,255,157,.06), transparent); border-color: var(--accent-dim); }
    .tab.active::after { content: ''; position: absolute; left: 0; right: 0; bottom: -1px; height: 2px; background: var(--accent);
      box-shadow: 0 0 12px var(--accent); }
    .tab:hover { color: var(--text); }

    main { padding: 0 20px; }
    .panel { display: none; padding-top: 16px; animation: rise .3s ease both; }
    .panel.active { display: block; }

    /* ── Переключатель языка ── */
    .lang-switch { margin-left: auto; display: flex; gap: 4px; }
    .lang-btn { padding: 6px 12px; border-radius: 5px; background: var(--panel); border: 1px solid var(--border);
      color: var(--muted); font-size: 11px; font-weight: 700; cursor: pointer; transition: all .15s ease; }
    .lang-btn.active { color: var(--accent); border-color: var(--accent-dim); background: rgba(0,255,157,.08); }
    .lang-btn:hover { color: var(--text); }

    /* ── Карточки/сетки ── */
    .grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 16px; }
    .card { background: linear-gradient(180deg, var(--panel), #0a0e15); border-radius: 8px; padding: 18px 20px;
      border: 1px solid var(--border); box-shadow: inset 0 1px 0 rgba(255,255,255,.02), 0 6px 22px rgba(0,0,0,.4);
      transition: border-color .2s ease, transform .2s ease; }
    .card:hover { border-color: var(--accent-dim); }
    .card h2 { color: var(--accent); font-size: 11px; margin-bottom: 14px; text-transform: uppercase; letter-spacing: 1px;
      display: flex; align-items: center; gap: 8px; }
    .card h2::before { content: ''; width: 12px; height: 2px; background: var(--accent); box-shadow: 0 0 8px var(--accent); }
    .stat { display: flex; justify-content: space-between; margin-bottom: 8px; gap: 10px; }
    .stat-label { color: var(--muted); }
    .stat-value { color: var(--text); font-weight: 700; text-align: right; }

    /* ── Алерты ── */
    #alerts { display: flex; flex-direction: column; gap: 8px; margin-bottom: 16px; }
    .alert { padding: 11px 16px; border-radius: 6px; font-size: 13px; border: 1px solid; border-left-width: 3px; }
    .alert.err { background: rgba(255,92,92,.06); border-color: var(--err); color: #ffabab; border-left-color: var(--err); }
    .alert.warn { background: rgba(255,180,84,.05); border-color: var(--warn); color: #ffc98f; border-left-color: var(--warn); }
    .alert.ok { background: rgba(0,255,157,.04); border-color: var(--ok); color: #a7ffd8; border-left-color: var(--ok); }

    /* ── Тулбар: поиск + фильтры ── */
    .toolbar { display: flex; flex-wrap: wrap; gap: 10px; align-items: center; margin-bottom: 14px; }
    .search { flex: 1; min-width: 200px; max-width: 340px; padding: 9px 13px; border-radius: 6px;
      border: 1px solid var(--border); background: var(--panel); color: var(--text); font-size: 13px; outline: none;
      transition: border-color .15s ease, box-shadow .15s ease; }
    .search:focus { border-color: var(--accent-dim); box-shadow: 0 0 0 2px rgba(0,255,157,.08); }
    select.filter { padding: 9px 11px; border-radius: 6px; border: 1px solid var(--border);
      background: var(--panel); color: var(--text); font-size: 12px; outline: none; cursor: pointer; }
    .chips { display: flex; gap: 6px; flex-wrap: wrap; }
    .chip { padding: 7px 14px; border-radius: 999px; background: var(--panel); border: 1px solid var(--border);
      color: var(--muted); font-size: 12px; cursor: pointer; user-select: none; transition: all .15s ease; }
    .chip.active { color: var(--accent); border-color: var(--accent-dim); background: rgba(0,255,157,.07); }
    ...
    th, td { text-align: left; padding: 11px 13px; border-bottom: 1px solid var(--border); font-size: 12px; white-space: nowrap; }
    th { color: var(--accent); font-size: 10.5px; text-transform: uppercase; letter-spacing: .8px; cursor: pointer; user-select: none; }
    th:hover { color: var(--text); }
    th .arr { font-size: 9px; margin-left: 4px; }
    td { color: var(--text); }
    tbody tr { transition: background .1s ease; }
    tr:hover td { background: rgba(0,255,157,.03); }
    .badge { padding: 3px 9px; border-radius: 999px; font-size: 10.5px; font-weight: 700; display: inline-block; letter-spacing: .3px; }
    .badge.up, .badge.active { background: rgba(0,255,157,.1); color: var(--ok); }
    .badge.ratelimited { background: rgba(255,180,84,.1); color: var(--warn); }
    .badge.error, .badge.dead { background: rgba(255,92,92,.1); color: var(--err); }
    .badge.unknown, .badge.untested { background: rgba(93,109,125,.15); color: var(--muted); }
    .badge.disabled, .badge.user-disabled { background: rgba(93,109,125,.08); color: var(--muted); }
    ...
    .mini-btn { padding: 5px 10px; border-radius: 5px; border: 1px solid var(--border); background: var(--panel2);
      color: var(--text); font-size: 11px; cursor: pointer; transition: all .15s ease; }
    .mini-btn:hover { border-color: var(--accent-dim); color: var(--accent); box-shadow: 0 0 10px rgba(0,255,157,.15); }
    .mini-btn.off { color: var(--warn); }

    /* ── Рекомендации ── */
    .rec { font-size: 12px; margin-bottom: 7px; line-height: 1.6; }
    .rec-badge { padding: 3px 9px; border-radius: 999px; font-size: 11px; font-weight: 700; margin-right: 6px; }
    .rec-badge.ok { background: rgba(0,255,157,.1); color: var(--ok); }
    .rec-badge.warn { background: rgba(255,180,84,.1); color: var(--warn); }
    .rec-badge { background: rgba(93,109,125,.15); color: var(--muted); }

    /* ── Экономия ── */
    .savings .savings-usd { color: var(--ok); font-size: 26px; text-shadow: 0 0 22px rgba(0,255,157,.4); }

    canvas { width: 100%; height: 160px; background: var(--panel); border-radius: 8px; border: 1px solid var(--border); }
    ...
    .task-bar .bar { flex: 1; height: 9px; background: var(--panel2); border-radius: 5px; overflow: hidden; }
    .task-bar .fill { height: 100%; border-radius: 5px; box-shadow: 0 0 10px rgba(0,255,157,.2); }
    .task-bar .num { width: 84px; text-align: right; }

    /* ── Setup ── */
    .setup-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(340px, 1fr)); gap: 14px; }
    .key-card { background: linear-gradient(180deg, var(--panel), #0a0e15); border-radius: 8px; padding: 18px; border: 1px solid var(--border); }
    .key-card h3 { color: var(--accent); font-size: 14px; margin-bottom: 8px; }
    .key-card .desc { color: var(--muted); font-size: 12px; margin-bottom: 10px; line-height: 1.5; }
    .key-card ol { color: var(--muted); font-size: 11px; padding-left: 18px; margin-bottom: 10px; }
    ...
    .key-input { flex: 1; padding: 9px; border-radius: 6px; border: 1px solid var(--border); background: var(--bg); color: var(--text); font-size: 12px; font-family: monospace; }
    .btn { padding: 9px 15px; border-radius: 6px; border: none; cursor: pointer; font-size: 12px; font-weight: 700; transition: all .15s ease; }
    .btn-validate { background: var(--panel2); color: var(--accent); border: 1px solid var(--border); }
    .btn-save { background: var(--accent); color: #04110b; box-shadow: 0 0 18px rgba(0,255,157,.3); }
    .btn-save:hover { box-shadow: 0 0 26px rgba(0,255,157,.5); }
    ...
    @media (max-width: 640px) {
      ...
      .kpi-chip { font-size: 10px; padding: 5px 9px; }
...
<body>
  ...
  <script>
    ...
    function renderRpm(rpmData) {
      ...
      grad.addColorStop(0, 'rgba(0,255,157,.22)'); grad.addColorStop(1, 'rgba(0,255,157,0)');
      ctx.fillStyle = grad;
      ctx.fill();
    ...
    function renderContext(sum) {
      ...
      if (sum.tasks && sum.taskTotal > 0) {
        var colors = { coding: '#00ff9d', reasoning: '#b388ff', search: '#00e5ff', chat: '#ffb454', design: '#ff6ec7' };
