...
<head>
  ...
  <title>Freegate — Панель управления</title>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .side-user .info .version { font-size: 10.5px; font-weight: 500; color: var(--accent-dim); margin-left: 2px; }
    ...
    .mini-btn.on { color: var(--ok); border-color: rgba(74,222,128,.35); }
    .mini-btn.off { color: var(--warn); border-color: rgba(251,191,36,.35); }
    .test-res { font-size: 11px; margin-left: 6px; min-width: 56px; }
    .test-res.testing { color: var(--accent-dim); }
    .test-res.ok { color: var(--ok); }
    .test-res.fail { color: var(--err); }
    ...
    .key-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
    .key-head h3 { margin: 0; }
    .key-actions { display: flex; gap: 8px; margin-top: 10px; }
    .key-link { color: var(--accent-ink); text-decoration: none; }
  ...
  <div class="layout">
    <aside id="sidebar">
      ...
      <nav id="tabs">
        ...
        <div class="tab" data-i18n="tabDiag" onclick="showTab('diag')"></div>
      ...
      <div class="side-foot">
        <div class="side-user">
          ...
          <div class="info">
            <div class="email">Freegate <span id="fgVersion" class="version"></span></div>
            <div class="role" data-i18n="sideRole"></div>
          </div>
        </div>
      </div>
    </aside>
    ...
    <section id="panel-diag" class="panel">
      <div id="diagSummary" class="grid"></div>
      <div class="card" style="margin-top:16px;">
        <h2 data-i18n="diagErrors"></h2>
        <div class="table-wrap"><table id="diagErrorsTable"></table></div>
      </div>
    </section>
    ...
    <section id="panel-setup" class="panel">
      ...
      <div class="card" style="margin-bottom:14px;">
        ...
        <div class="opt-row"><span data-i18n="optPreference"></span>
          <select id="optPreference" class="filter" onchange="saveOpts()">
            <option value="free-first" data-i18n="prefFreeFirst"></option>
            <option value="paid-first" data-i18n="prefPaidFirst"></option>
            <option value="paid-fallback" data-i18n="prefPaidFallback"></option>
          </select>
          <span class="opt-hint" id="optPreferenceState"></span>
        </div>
        <div class="opt-desc" data-i18n="optPreferenceDesc"></div>
  ...
  <script>
    ...
    var I18N = {
      ru: {
        ...
        tabDiag: 'Диагностика',
        diagErrors: 'Ошибки сегодня (по кодам и провайдерам)', diagProviders: 'Провайдеры и статус',
        diagSuccessRate: 'Успешность', diagCategory: 'Класс', diagHits: 'Ошибок',
        diagInvalid: 'Невалидные (400/422) — не лимит, чинится в коде', diagLimit: 'Лимит/429 — выгорание дневных лимитов',
        diagTimeout: 'Таймаут/сетевые', diagAuth: '401/402/403 — ключ/баланс', diagFix: 'Фикс',
        diagFixSanitize: 'санитизация reasoning_effort', diagFixAuth: 'исключение из роутинга',
        diagFixLimit: 'предохранитель выгорания', diagFixTimeout: 'дольше холдить', diagNone: 'нет',
        ...
        optPreference: 'Предпочтение free↔paid',
        prefFreeFirst: 'free-first (бесплатные в приоритете)', prefPaidFirst: 'paid-first (свои платные в приоритете)', prefPaidFallback: 'paid-fallback (платные — как запас)',
        optPreferenceDesc: 'Как использовать твои платные ключи рядом с бесплатной базой: бесплатные всегда, свои платные в первую очередь, или платные только когда бесплатные упали. У платного провайдера в каталоге стоит флаг paid или free=false.',
        ...
        enabledShort: 'Вкл', disabledShort: 'Выкл',
        ...
        saveKey: 'Сохранить ключ', savedKey: 'Ключ сохранён. Перезапусти Freegate.',
      ...
      en: {
        ...
        tabDiag: 'Diagnostics',
        diagErrors: 'Today errors (by code & provider)', diagProviders: 'Providers and status',
        diagSuccessRate: 'Success rate', diagCategory: 'Class', diagHits: 'Errors',
        diagInvalid: 'Invalid (400/422) — not a limit, fixable in code', diagLimit: 'Rate/429 — daily limit burnout',
        diagTimeout: 'Timeout/network', diagAuth: '401/402/403 — key/balance', diagFix: 'Fix',
        diagFixSanitize: 'reasoning_effort sanitization', diagFixAuth: 'exclude from routing',
        diagFixLimit: 'burnout guard', diagFixTimeout: 'backoff longer', diagNone: 'none',
        ...
        optPreference: 'Free↔paid preference',
        prefFreeFirst: 'free-first (free in priority)', prefPaidFirst: 'paid-first (your paid keys in priority)', prefPaidFallback: 'paid-fallback (paid as a fallback)',
        optPreferenceDesc: 'How to use your paid keys alongside the free base: free always, your paid first, or paid only when free fail. Mark a paid provider with the paid flag (free=false) in the catalog.',
        ...
        enabledShort: 'On', disabledShort: 'Off',
        ...
        saveKey: 'Save key', savedKey: 'Key saved. Restart Freegate.',
    ...
    // ── табы ──
    var _tabByKey = { overview: 'tabOverview', providers: 'tabProviders', models: 'tabModels', context: 'tabContext', diag: 'tabDiag', setup: 'tabSetup' };
    ...
    function refresh() {
      Promise.all([
        ...
        var v = document.getElementById('fgVersion');
        if (v) v.textContent = _statsData.version ? ('· v' + _statsData.version) : '';
        renderKpi();
        renderAlerts();
        renderOverviewCards();
        renderRpm(_rpmData);
      ...
      rows.forEach(function (m) {
        ...
        html += '<tr>' +
          ...
          '<td class="model-actions">' +
            '<span class="mini-btn ' + (on ? 'on' : 'off') + '" onclick="toggleModel(\\'' + esc(m.key) + '\\')" title="' + (on ? t('disable') : t('enable')) + '">' + (on ? t('enabledShort') : t('disabledShort')) + '</span>' +
            '<span class="mini-btn" onclick="testModelLive(\\'' + esc(m.key) + '\\', this)">' + t('testNow') + '</span>' +
            '<span id="testres-' + esc(m.key) + '" class="test-res"></span>' +
          '</td></tr>';
      });
    ...
    function testModelLive(key, el) {
      var res = document.getElementById('testres-' + key);
      if (res) { res.className = 'test-res testing'; res.textContent = t('testing'); }
      fetch(_api('/v1/models/' + encodeURIComponent(key) + '/test'), { method: 'POST' })
        .then(function (r) { return r.json(); })
        .then(function (d) {
          var ok = !!d.ok;
          if (res) { res.className = 'test-res ' + (ok ? 'ok' : 'fail'); res.textContent = ok ? (d.latencyMs + 'ms ✓') : (d.status || 'FAIL'); }
          fetchModels();
        })
        .catch(function (e) { if (res) { res.className = 'test-res fail'; res.textContent = 'ERR'; } });
    }
    ...
    // ── Диагностика: успешность по кодам, классы ошибок, статусы провайдеров ──
    function renderDiag() {
      var s = _statsData || {};
      var today = s.today || {};
      var errors = s.errors || {};
      var health = s.health || {};
      var upCnt = 0, errCnt = 0, ratelimitedCnt = 0;
      for (var ek in health) {
        var v = health[ek];
        if (v.status === 'up') upCnt++;
        else if (v.status === 'ratelimited') ratelimitedCnt++;
        else if (v.status === 'error') errCnt++;
      }
      var total = today.requests || 0;
      var successRate = total > 0 ? Math.round((today.success / total) * 100) : 100;
      document.getElementById('diagSummary').innerHTML =
        '<div class="card"><h2>' + t('diagSuccessRate') + '</h2><div class="stat"><span class="stat-value" style="font-size:28px;color:' + limitColor(successRate) + '">' + successRate + '%</span>' +
        '<span class="stat-label">' + today.success + ' / ' + total + '</span></div></div>' +
        '<div class="card"><h2>Up</h2><div class="stat"><span class="stat-value">' + upCnt + '</span><span class="stat-label">status up</span></div></div>' +
        '<div class="card"><h2>Limit</h2><div class="stat"><span class="stat-value" style="color:var(--warn)">' + ratelimitedCnt + '</span><span class="stat-label">429 (выгорели лимиты)</span></div></div>' +
        '<div class="card"><h2>Error</h2><div class="stat"><span class="stat-value" style="color:var(--err)">' + errCnt + '</span><span class="stat-label">' + t('diagAuth') + '</span></div></div>';
      var rows = Object.keys(errors).map(function (k) {
        var st = health[k] || {};
      ...
      document.getElementById('diagErrorsTable').innerHTML = rows.length
        ? '<tr><th>' + t('thKey') + '</th><th>count</th><th>status</th><th>reason</th></tr>' + rows.join('')
        : '<div class="stat">' + t('diagNone') + '</div>';
    }
    ...
    async function loadOpts() {
      try {
        ...
        document.getElementById('optPreference').value = c.routing.preference || 'free-first';
    ...
    function saveOpts() {
      var body = {
        ...
        routing: { strategy: document.getElementById('optRouting').value, preference: document.getElementById('optPreference').value },
      };
    ...
    function renderSetup(data) {
      ...
      for (var envVar in data.groups) {
        ...
        var stClass = maskedVal ? 'status-ok' : 'status-wait';
        html += '<div class="key-card" id="card-' + envVar + '">';
        html += '<div class="key-head"><h3>' + esc(name) + '</h3><span id="badge-' + envVar + '" class="badge ' + (maskedVal ? 'active' : 'disabled') + '">' + (maskedVal ? '✓' : '—') + '</span></div>';
        ...
        html += '<button class="btn btn-save" data-save="' + envVar + '">' + t('saveKey') + '</button>';
        html += '</div>';
        html += '<div id="status-' + envVar + '" class="' + stClass + '" style="margin-top: 6px;">' + (maskedVal ? t('keySet') + ' (' + maskedVal + ')' : t('keyMissing')) + '</div>';
        html += '<a href="' + info.url + '" target="_blank" class="key-link" style="font-size: 11px;">' + esc(info.url) + ' ↗</a>';
        html += '</div>';
      }
      grid.innerHTML = html;
    }
    document.addEventListener('click', function (e) {
      ...
      if (e.target.dataset && e.target.dataset.save) saveKey(e.target.dataset.save);
    });
    ...
    async function saveKey(envVar) {
      ...
      try {
        var res = await fetch(_api('/v1/setup/keys'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(((_k) => { var o = {}; o[_k] = key; return o; })(envVar))
        });
        var result = await res.json();
        if (result.ok) {
          status.className = 'status-ok'; status.textContent = t('savedKey');
          var badge = document.getElementById('badge-' + envVar);
          if (badge) { badge.className = 'badge active'; badge.textContent = '✓'; }
        } else {
          status.className = 'status-err'; status.textContent = (result.error || t('saveErr'));
        }
      } catch (e) { status.className = 'status-err'; status.textContent = e.message; }
    }
