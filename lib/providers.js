...
// Minimal fallback config so the proxy NEVER dies from a corrupt/missing config.
// Real providers get merged over this from config.json when it parses.
const DEFAULT_CONFIG = {
  port: 4000,
  auth: 'free-llm-proxy-2024',
...
function loadConfig() {
  try {
    ...
    // Sanity: must have a providers object with at least one entry
    if (!parsed || typeof parsed !== 'object' || !parsed.providers || Object.keys(parsed.providers).length === 0) {
      console.error(`Config invalid (no providers) — using defaults: ${CONFIG_PATH}`);
      return DEFAULT_CONFIG;
    }
    return parsed;
  } catch (err) {
    console.error(`Config corrupt — using defaults: ${CONFIG_PATH} (${err.message})`);
    return DEFAULT_CONFIG;
  }
}
