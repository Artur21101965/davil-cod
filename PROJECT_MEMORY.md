...
- **Window-aware роутинг обходит primary-target** (server.js `MODEL_MAP` ставит целевую модель первой): для `tier-s` гигантский промпт всегда пробует codestral вне зависимости от `win >= requestTokens`. Фильтр применяется только к fallback-пулу.
